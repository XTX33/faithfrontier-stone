...
- **No CSS in markdown**: Do not paste CSS or ````html`n&lt;style&gt;`n```` blocks into markdown content. Keep styles in `/assets/css/*.css` (homepage-only overrides live in `assets/css/home.css`).
